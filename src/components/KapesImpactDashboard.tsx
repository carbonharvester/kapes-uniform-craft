      ...
      <div className="container mx-auto px-4">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-4 gap-6 mb-20">
            {impactMetrics.map((metric, index) => <Card key={index} className="text-center border-border/50 shadow-sm hover:shadow-md transition-all duration-300">
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center mb-20">
            <div className="space-y-8">
              ...
              <div className="space-y-4">
                {dashboardFeatures.map((feature, index) => <button key={index} onClick={() => setSelectedFeature(index)} className={`flex gap-4 p-4 rounded-xl border transition-all text-left w-full ${selectedFeature === index ? 'bg-primary/5 border-primary/20 shadow-sm' : 'bg-card/50 hover:bg-card border-border/50'}`}>
                    <div className="flex-shrink-0">
                      <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${selectedFeature === index ? 'bg-primary text-primary-foreground' : 'bg-primary/10'}`}>
                        <feature.icon className={`h-5 w-5 ${selectedFeature === index ? 'text-primary-foreground' : 'text-primary'}`} />
                      </div>
                    </div>
                    <div>
            ...
            <div className="relative">
              <Card className="relative overflow-hidden border-border/50 shadow-lg">
                <div className="aspect-video bg-muted rounded-lg overflow-hidden">
                  <img src={dashboardFeatures[selectedFeature].screenshot} alt={`${dashboardFeatures[selectedFeature].title} screenshot`} className="w-full h-full object-cover transition-all duration-500" />
                </div>
              </Card>
            </div>
          </div>
